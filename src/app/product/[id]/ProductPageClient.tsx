...
type FormErrors = Partial<Record<"name" | "address" | "size" | "color", string>>;
...
export default function ProductPageClient({ product }: { product: Product }) {
  ...
  // Switch color = switch product variant
      ...
      <div className="min-h-screen flex items-center justify-center px-6" style={{ background: "var(--cream)" }}>
        <div className="text-center max-w-md">
          <div className="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-8 text-4xl"
            style={{ background: "var(--accent)", boxShadow: "0 0 50px rgba(74,87,56,0.35)" }}>
            ✓
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="lg:sticky lg:top-28">
              {/* Main image */}
              <div
                className="rounded-3xl overflow-hidden flex items-center justify-center mb-4 aspect-square max-w-lg mx-auto"
                style={{
              ...
              {/* Thumbnail strip — all color variants */}
            ...
            <div>
              ...
              <p className="text-base leading-loose mb-8"
                style={{ color: "var(--muted)", fontFamily: "Cairo, sans-serif", lineHeight: "2" }}>
              ...
              <form onSubmit={handleSubmit} noValidate className="space-y-6">
                <h2 className="text-2xl font-black mb-6" style={{ color: "var(--charcoal)", fontFamily: "Cairo, sans-serif" }}>
                  أكمل طلبك
                </h2>
                    ...
                    {allProducts.map((p) => (
                      <button
                        ...
                        style={{
                          ...
                          boxShadow: selectedColor.id === p.id
                ...
                <div>
                  ...
                  {errors.size && <p className="mt-2 text-sm" style={{ color: "#c0392b", fontFamily: "Cairo, sans-serif" }}>⚠ {errors.size}</p>}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-0 w-fit rounded-xl overflow-hidden" style={{ border: "2px solid var(--border)" }}>
                    <button type="button" onClick={() => setQuantity(q => Math.max(1, q - 1))}
                      className="w-12 h-12 flex items-center justify-center text-xl font-bold transition-colors hover:bg-[var(--sand-light)] cursor-pointer"
                      style={{ color: "var(--charcoal)" }}>−</button>
                    <span className="w-12 h-12 flex items-center justify-center font-bold text-lg" style={{ color: "var(--charcoal)", fontFamily: "Cairo, sans-serif" }}>
                      {quantity}
                    </span>
                ...
                {/* Name */}
                <div>
                  <label htmlFor="prod-name" className="block text-sm font-bold mb-2" style={{ color: "var(--charcoal)", fontFamily: "Cairo, sans-serif" }}>
                  ...
                  <input
                    ...
                    disabled={isLoading}
                  />
                  {errors.name && <p className="mt-2 text-sm" style={{ color: "#c0392b", fontFamily: "Cairo, sans-serif" }}>⚠ {errors.name}</p>}
                </div>

                {/* Address */}
                <div>
                  <label htmlFor="prod-address" className="block text-sm font-bold mb-2" style={{ color: "var(--charcoal)", fontFamily: "Cairo, sans-serif" }}>
                  ...
                  <textarea
                    ...
                    disabled={isLoading}
                  />
                  {errors.address && <p className="mt-2 text-sm" style={{ color: "#c0392b", fontFamily: "Cairo, sans-serif" }}>⚠ {errors.address}</p>}
                </div>

                {/* Summary chip */}
                <div className="flex flex-wrap gap-2">
                  {[selectedColor.color, selectedSize, `${quantity} قطعة`, selectedColor.price].map((val) => (
                    <span key={val} className="px-4 py-1.5 rounded-full text-sm font-bold"
                      style={{ background: "var(--sand-light)", color: "var(--charcoal)", border: "1px solid var(--sand-dark)", fontFamily: "Cairo, sans-serif" }}>
                      {val}
                    </span>
                  ))}
                </div>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="btn-primary w-full text-white font-black py-5 rounded-2xl text-xl cursor-pointer flex items-center justify-center gap-3 disabled:opacity-70"
                  style={{ fontFamily: "Cairo, sans-serif" }}
                >
                  {isLoading ? (
                    <>
                ...
                <p className="text-center text-xs" style={{ color: "var(--muted)", fontFamily: "Cairo, sans-serif" }}>
                  🔒 بيانتاك آمنة — لن نشاركها مع أي طرف ثالث
                </p>
              </form>
            </div>
          </div>
